...
function addCard(clickedInterest) {
    let card = document.createElement('div');
    card.className = 'card';
    card.innerHTML = "<div class='image-area'><img src='Twitter_Logo_Blue.png' class='card-img img-fluid img-thumbnail'\
        alt='generated tweet' width: 50%; height: auto;>\
        </div>";
    let closeButton = document.createElement('div');
    closeButton.className ='btn-group my-auto';
    closeButton.innerHTML = "<button type = 'button' class='btn btn-danger btn-circle btn-sm d-flex' onclick = 'removeCard(this.parentNode)' >&#x2715</button>\
        </div>";
    clickedInterest.parentNode.appendChild(card);
    card.append(closeButton);
    let newID;
    while(true) {
        newID = Math.floor(Math.random() * 10000);
        if (tweetIDs.every(function (val) {return val!=newID})) {
            break;
        }
    }
    tweetIDs.push(newID)
    card.id = "tweetID" + clickedInterest.id.toString() + "-" + newID; 
    ...
    // header.innerText = "interest" + clickedInterest.id.charAt(clickedInterest.id.length - 1);
    header.className = 'card-title';
    cardBody.append(header);
    
    let body = document.createElement('div');
    body.className = "card-text text-muted";
    body.innerText = "twt#" + newID; 
    cardBody.append(body) 
}

function removeCard(card) {
    card.parentNode.remove();
    tweetIDs.splice(tweetIDs.indexOf(card.parentNode.id.split('-')[1]),1);
}
...
function publishCard(button) {
    ...
    buttonParent.innerHTML = "<button type='button' class='btn btn-danger btn-circle btn-sm d-flex' onclick='removeCard(this)'>X</button>\
                <img src='" + imgSRC + "' class='card-img-top'\
                    alt='" + interestName + "'>\
                <button type = 'button' class='btn btn-primary btn-circle btn-twt d-flex' id='twitButton" + cardChar + "' onclick ='addCard(this)' >+</button >\
